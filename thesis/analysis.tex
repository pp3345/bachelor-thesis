...
During module startup, the \gls{PMU} is reset to a default state and the performance counter overflow interrupt vector is set in all core's \gls{APIC}'s \glspl{LVT}. Notably, this degrades the functionality of \gls{Linux}'s \texttt{perf} subsystem as \texttt{perf} partially relies on using the \gls{PMU}.
