...
\newacronym[description={In general, ASLR is a technique that tries to prevent malicious code from being able to guess the memory locations of symbols or data in a process. KASLR implements this for the Linux kernel}]{KASLR}{KASLR}{Kernel Address Space Layout Randomization}
\newacronym[description={NVMe is a modern device interface specification designed for Solid State Drives attached via \gls{PCIe}, designed to reach low latencies and high speeds compared to earlier interfaces}]{NVMe}{NVMe}{Non-Volatile Memory Express}\glsunset{NVMe}
