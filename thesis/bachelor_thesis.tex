% Contributors:

% Jan Stoess <stoess@ira.uka.de>
% Simon Kellner <kellner@kit.edu>
% Konrad Miller <miller@kit.edu>
%
\documentclass[12pt, a4paper]{book}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{changepage}
\usepackage{xspace,ifthen,epsfig}
\usepackage[backend=biber,style=numeric,giveninits=true,maxcitenames=1]{biblatex}
\usepackage{color}
\usepackage{fancybox}
%\usepackage{pdfpages}
\usepackage{setspace}
\usepackage{subcaption}
\usepackage{longtable} 
\usepackage{tabularx} 
\usepackage{ltxtable} 
\usepackage{times}
\usepackage{url}
\usepackage{listings}
\usepackage{mathtools}
\usepackage[american,ngerman]{babel}
\usepackage{fancyhdr}
\usepackage{libertine}
\usepackage{newtxmath}
\usepackage[scaled=0.78]{beramono}
\usepackage[T1]{fontenc}
\usepackage{styles/kitthesiscover}
\usepackage[%dvipdfm,
   pdfauthor={Yussuf Khalil},
   pdftitle={Analysis and Optimization of Dynamic Voltage and Frequency Scaling for AVX Workloads Using a Software-Based Reimplementation},
   pdfsubject={Bachelor's Thesis},
   pdfkeywords={Operating Systems, Linux, Intel, x86, AVX, DVFS, Heterogeneous Workloads, Dim Silicon},
   hidelinks
]{hyperref}
\usepackage{cleveref}
\usepackage[binary-units=true]{siunitx}
\usepackage[automake,ucmark]{glossaries}
\usepackage{pgfplots}
\pgfplotsset{compat=1.16}
\usepgfplotslibrary{statistics}
\usepackage{tikz}
\usetikzlibrary{decorations.pathreplacing,shapes,positioning}
\usepackage{marvosym}
\usepackage{minted}
\usepackage[autostyle]{csquotes}
\usepackage{sectsty}
\usepackage[clock]{ifsym}
\usepackage{emptypage}
\usepackage{microtype}

\setlength{\headheight}{15pt}

\setacronymstyle{long-short-desc}

\definecolor{kitgreenexcl}{cmyk}{1.0,  0.0,  0.6, 0.0}
\definecolor{kitblue}     {cmyk}{0.8,  0.5,  0.0, 0.0}
\definecolor{kitgreen}    {cmyk}{0.6,  0.0,  1.0, 0.0}
\definecolor{kityellow}   {cmyk}{0.0,  0.05, 1.0, 0.0}
\definecolor{kitorange}   {cmyk}{0.0,  0.45, 1.0, 0.0}
\definecolor{kitbrown}    {cmyk}{0.35, 0.5,  1.0, 0.0}
\definecolor{kitred}      {cmyk}{0.25, 1.0,  1.0, 0.0}
\definecolor{kitpurple}   {cmyk}{0.25, 1.0,  0.0, 0.0}
\definecolor{kitcyan}     {cmyk}{0.9,  0.05, 0.0, 0.0}
\definecolor{kitgrey}	  {cmyk}{0.0,  0.0,  0.0, 0.15}
\definecolor{kitdarkgrey} {cmyk}{0.0,  0.0,  0.0, 0.5}

\usemintedstyle{colorful}

\allsectionsfont{\sffamily}

\fancyhead[ER]{\sffamily\selectfont\slshape\rightmark}
\fancyhead[OL]{\sffamily\selectfont\slshape\rightmark}
\fancyhead[OR,EL]{}
\renewcommand{\headrulewidth}{0pt}
\pagestyle{fancy}

%\raggedbottom
\newcommand{\todo}[1]{{\texttt{[#1]}}}
\newcommand{\code}[1]{{\tt \small{#1}}}
\newcommand{\evenindent}[2]{\ifodd #1 \else \hspace*{#2} \fi}

% https://tex.stackexchange.com/a/47797
\tikzset{ref/.style={insert path={%
			coordinate [pos=0,xshift=-0.5\pgflinewidth,yshift=-0.5\pgflinewidth] (#1 south west)
			coordinate [pos=1,xshift=0.5\pgflinewidth,yshift=0.5\pgflinewidth]   (#1 north east)
			coordinate [pos=.5] (#1 center)
			(#1 south west |- #1 north east)     coordinate (#1 north west)
			(#1 center     |- #1 north east)     coordinate (#1 north)
			(#1 center     |- #1 south west)     coordinate (#1 south)
			(#1 south west -| #1 north east)     coordinate (#1 south east)
			(#1 center     -| #1 south west)     coordinate (#1 west)
			(#1 center     -| #1 north east)     coordinate (#1 east)
}}}

\input{glossary}
\addbibresource{bachelor_thesis.bib}

\begin{document}
\frontmatter
\unitlength1cm

\selectlanguage{american}


\input{bachelor_thesis_title}

\input{declaration}

\input{abstract}

\mainmatter
\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{Contents}
{\sffamily\tableofcontents}

\input{introduction}
\input{background}
\input{analysis}
\input{design}
\input{evaluation}
\input{conclusion}

\backmatter
%\chapter{Bibliography}

\phantomsection
\addcontentsline{toc}{chapter}{Glossary}
\printglossaries

\phantomsection
\addcontentsline{toc}{chapter}{Bibliography}
\printbibliography
\end{document}
